...
const categoryHeroGradients = {
  ...
  merchandise: 'linear-gradient(135deg, #1a0c0c 0%, #4a1e1e 50%, #7a4040 100%)',
  'packaged-coffee-tea': 'linear-gradient(135deg, #1a2208 0%, #3a4e18 50%, #5a6e30 100%)',
};
...
export default function ShopCategoryPage({ slug }) {
  ...
  const heroGradient = categoryHeroGradients[slug] || categoryHeroGradients.coffees;
  ...
  return (
    <main className="cat-page">
      ...
      <div
        className="cat-hero"
        style={{ background: `url('/Pattern.png') center/600px repeat, ${heroGradient}` }}
      >
      ...
      <div className="shop-tabs-wrap">
        <div className="shop-tabs">
          <Link to="/shop" className="shop-tab cat-back-tab">← {t.shop.backToShop}</Link>
      ...
      <div className="cat-grid-wrap">
        <div className="product-grid">
          {category.products.map(product => (
            <article className={`product-card ${slug}`} key={product.name}>
              <div
